...
async function run(rootPath: string) {
  ...
  const options: depcheck.Options = {
    ...
    // These come from `/// <reference types="X" />` directives that are satisfied by an `@types/*`
    // package (or root-provided types) rather than a runtime package literally named "X", so
    // depcheck cannot match them against the workspace's declared dependencies.
    ignoreMatches: ["~", "node", "foxglove__web", "wicg-file-system-access"],
  };
