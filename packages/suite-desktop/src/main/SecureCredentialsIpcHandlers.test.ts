...
describe("registerSecureCredentialsIpcHandlers", () => {
  it("allows registered main-frame senders and forwards all operations", async () => {
    ...
    await expect(
      handlers.get("secureCredentials:get")?.(event, "agent.profile.profile-123.llmApiKey"),
    ).resolves.toEqual({
      code: "backend-unavailable",
      ok: false,
    });
    await expect(
      handlers.get("secureCredentials:set")?.(event, "agent.llmApiKey", "new-secret"),
    ...
    expect(getCredential).toHaveBeenCalledWith("agent.profile.profile-123.llmApiKey");
    expect(setCredential).toHaveBeenNthCalledWith(1, "agent.llmApiKey", "new-secret");
