...
export function useDraggedMessagePath(value: string | undefined): DraggedMessagePath | undefined {
  ...
  return useMemo(() => {
    ...
    // back to the field-selector heuristic when the schema structure cannot be resolved or when
    // building the structures throws (e.g. an unresolvable datatype in the schema).
    ...
    if (hasFieldSelector && topic?.schemaName != undefined && datatypes.size > 0) {
      try {
        ...
        log.warn(
          `Failed to resolve datatype structure for schema "${topic.schemaName}"; ` +
            `falling back to field-selector leaf classification.`,
          error,
        );
      }
    }
