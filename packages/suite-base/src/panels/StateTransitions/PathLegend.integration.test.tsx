...
describe("PathLegend drag source integration (real HTML5 backend)", () => {
  ...
  it("makes the series label a native draggable element", () => {
    // Given a State Transitions legend with one configured series, rendered with the real HTML5
    ...
    // When locating the series label button
    const button = screen.getByTestId("edit-topic-button-0");

    // Then it is a native draggable element (the MUI Button ref forwarded to the DOM node)
    expect(button.getAttribute("draggable")).toBe("true");
  });

  it("does not make the placeholder row draggable", () => {
    // Given a State Transitions legend with no configured series (placeholder row only)
    render(
      <DndProvider backend={HTML5Backend}>
        <MockPanelContextProvider id="panel-a">
          <PathLegend
    ...
    // When locating the placeholder "add series" button
    const button = screen.getByTestId("edit-topic-button-0");

    // Then it is not draggable
