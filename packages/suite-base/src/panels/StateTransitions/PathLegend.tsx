...
type PathLegendRowProps = Readonly<{
...
/**
 * A single row in the State Transitions legend. Renders the series label (which doubles as a
 * message-path drag source so the series can be dragged onto another timeseries panel) and a
 * delete button, or an "add series" placeholder when there are no configured paths.
 */
function PathLegendRow({
  ...
  return (
    <div data-testid={`row-${index}`} className={classes.row}>
...
/**
 * Legend overlay for the State Transitions panel. Renders one {@link PathLegendRow} per configured
 * series (or a single placeholder row to add the first series) and wires up editing and deletion.
 */
...
export const PathLegend = React.memo(function PathLegend(props: PathLegendProps) {
  ...
  const { classes } = useStyles({ heightPerTopic, cursor: undefined, isDragging: false });
