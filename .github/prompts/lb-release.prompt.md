...
6. After merge, no manual workflow dispatch is needed:
   - `release.yml` runs automatically
   - the resulting GitHub Release triggers `post-release.yml`
   - the same release event also triggers `release-sync.yml`
...
- [ ] If the sync PR has merge conflicts, resolve them manually and merge with a **MERGE COMMIT** only — never squash or rebase.
